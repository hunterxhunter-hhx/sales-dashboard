...
const testCode = [
  ...
  "if (ORDER_UNIT_PRICE !== 2980) {",
  "  throw new Error(`Prediction unit price mismatch: got ${ORDER_UNIT_PRICE}, expected 2980`);",
  "}",
  ...
  "  if (row.gmv !== row.orders * 2980) {",
  "    throw new Error(`Prediction GMV mismatch for ${row.name}: got ${row.gmv}, expected ${row.orders * 2980}`);",
  "  }",
  "});",
